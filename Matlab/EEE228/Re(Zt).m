...
for ix0 = 1:length(RE)
    Zt_mesh = squeeze(Zt(ix0,:,:));
    subplot(2,1,1);
    mesh(C1,C2,real(Zt_mesh));
    title('surface plot $Re\{zt\}$' , 'Interpreter' , 'latex')
    x1abe1('$C_(1)$' , 'Interpreter' , 'latex')
    y1abe1('$C_(2)$' , 'Interpreter' , 'latex')
    z1abe1('$re_(zt)$' , 'Interpreter' , 'latex')
    subplot(2,1,2);
    mesh(C1,C2,imag(Zt_mesh));
 title('surface plot $Re\{zt\}$' , 'Interpreter' , 'latex')
    xlabel('$C_(1)$' , 'Interpreter' , 'latex')
    ylabel('$C_(2)$' , 'Interpreter' , 'latex')
    zlabel('$re_(zt)$' , 'Interpreter' , 'latex')
    texto = strcat('$R_(E)=' , num2str(RE(ix0)), '\Omega$');
    sgtitle(texto,'Interpreter' , 'latex');
    pause(0.1);
end